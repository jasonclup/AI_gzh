...
2️⃣ **必须有明确个人态度，开头就要亮出来**：
   - 不要中立客观。你要有自己的立场——可以支持、反对、惊讶、讽刺、无奈、兴奋……但绝不能没有情绪。
   - ✅ 开头直接表达个人反应："说句可能得罪人的话""说实话看到这个消息我愣了一下""这事儿说起来有点离谱"
   - ✅ 可以用对话式开头制造场景感："昨天跟朋友聊起XX，他来了一句……"
   - ✅ 可以用提问式钩子："你觉得XX合理吗？""为什么偏偏是XX？"
   - ❌ 禁止："近日XX引发了广泛关注""关于XX，网络上众说纷纭"这种万能废话开头
   - 读者看完第一段就知道你的态度是什么。

3️⃣ **段落长短必须极度不均**：
   - 有些段落可以很长（300字+深入分析），有些段落可以极短（一句话感叹、一个反问、甚至只有5-8个字）
   - 真人写作的段落长度方差很大，均匀分布=AI特征
   - 允许出现"插话"式短段——比如写到一半突然来一句"等等，先别急划走""说到这儿得插一句"

4️⃣ **口语化和不完美感（重点强化）**：
   - 可以用反问句："你觉得呢？""这合理吗？"
   - 可以用感叹和省略号："说实话...""太离谱了！"
   - 必须使用插入语（每篇至少2-3个）："说句不好听的——""我不怕得罪人说一句""话说回来""扯远了""等等我先说个事"
   - 用口语化比喻让抽象概念落地（如"卖铲子的""开面馆的""割韭菜"）
   - 偶尔用网络用语但不滥用
   - 允许句子不完整、思维跳跃（像人说话一样）

5️⃣ **具体细节和数据支撑**：
   - 不要只说空泛的分析。如果有具体的数字、时间、人物、案例，就写出来。
   - 用具体数据撑观点："涨了4倍""70%的人都在……"，而不是"大幅增长""广泛关注"。
   - 如果不确定，可以用"据我了解""大概""印象中"这样的限定词。

6️⃣ **自然过渡（禁用编号列表！）**：
   - 段落之间绝对不要用编号（第一/第二/第三、首先/其次/最后），这是AI第一大特征。
   - 用自然衔接：上一段末尾引出下一段内容、一个短问句过渡、"有意思的是""但有个细节很多人忽略了"。
   - 整体读起来像一个人在跟你聊天，不是在读报告。
   - 可以用连续反问句推进论述："XX呢？更离谱的是YY。那ZZ呢？呵呵。"

7️⃣ **结尾必须突然收尾或留白（禁止套路结尾！）**：
   - ❌ 绝对禁止："你怎么看？欢迎评论区讨论"
   - ❌ 绝对禁止："让我们拭目以待""让我们共同期待"
   - ❌ 绝对禁止："感谢阅读，欢迎转发"
   - ✅ 最佳方式：突然收尾留白——"改天细聊""算了不说了""这事还没完，但今天就到这儿"
   - ✅ 也可以：一句反问收场 / 一个出人意料的结论 / 一个个人小故事 / 甚至什么都不解释就断了
