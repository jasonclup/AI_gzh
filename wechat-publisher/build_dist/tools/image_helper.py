...
# 全局：记录最近一次下载的图片MD5，用于检测限流重复返回
_last_download_md5 = [None]  # 用list以便在函数内修改

# 每张图之间的最小请求间隔（秒），防止Pollinations限流导致返回相同图片
_MIN_REQUEST_INTERVAL = 12  # 每张图间隔12秒，Pollinations免费版需要较长的冷却时间
_last_request_time = [0]      # 上次请求时间戳
...
def _download_image_post(prompt: str, output_path: str,
    ...
    """使用GET方式请求Pollinations AI文生图（2026-04-14改用GET避免POST限流）

    修复：
    - 改用GET方式（Pollinations标准API，限流比POST宽松得多）
    - 添加请求间隔控制
    - 添加相同图片检测（IP限流返回相同图片时自动重试）
    """
    ...
    now = _time.time()
    elapsed_since_last = now - _last_request_time[0]
    if elapsed_since_last < _MIN_REQUEST_INTERVAL:
        wait = _MIN_REQUEST_INTERVAL - elapsed_since_last + 1.0
        log.info(f"Rate limit guard: waiting {wait:.1f}s before next request...")
        _time.sleep(wait)

    try:
        # 缓存破坏：添加时间戳确保每次请求都生成新图
        cache_buster = int(_time.time() * 1000)
        effective_seed = seed or (cache_buster % 100000)

        # 使用 GET 方式构建URL（Pollinations 标准API）
        encoded_prompt = quote(prompt)
        url = (
            f"https://image.pollinations.ai/prompt/{encoded_prompt}"
            f"?width={width}&height={height}"
            f"&model={model}&seed={effective_seed}"
            f"&nologo=true&_cb={cache_buster}"
        )
        ...
        log.info(f"GET generating image: prompt='{prompt[:60]}...' size={width}x{height} model={model} seed={effective_seed}")
        start = _time.time()
        ...
        with urllib.request.urlopen(req, timeout=timeout) as response:
            ...
            # 检测是否为Pollinations限流占位图
            if _is_placeholder_image(output_path):
                log.warning(f"Detected placeholder image, deleting and returning False")
                _clean_placeholder_file(output_path)
                return False

            # === 关键：检测是否和上次返回了相同的图片 ===
            current_md5 = hashlib.md5(data).hexdigest()[:12]
            if _last_download_md5[0] is not None and current_md5 == _last_download_md5[0]:
                log.warning(
                    f"RATE-LIMIT DETECTED: got SAME image (MD5={current_md5}) as previous! "
                    f"Pollinations returned cached/throttled result."
                )
                os.remove(output_path)
                return False
            _last_download_md5[0] = current_md5

            elapsed = _time.time() - start
            size_kb = len(data) // 1024
            log.info(f"GET image OK: {size_kb}KB in {elapsed:.1f}s MD5={current_md5} -> {output_path}")
            return True
        ...
        if 'timeout' in err_str or 'timed out' in err_str:
            raise
        log.error(f"GET download failed: {e}")
        return False
...
def _generate_with_pollinations(prompt_text: str, output_path: str, 
    ...
    for model_idx, try_model in enumerate(models_to_try):
        ...
        for retry in range(RETRY_MAX):
            ...
            # 重试时增加等待时间（指数退避 + 最小间隔）
            if retry > 0:
                wait_time = RETRY_DELAY_BASE * (2 ** (retry - 1)) + _MIN_REQUEST_INTERVAL
                log.info(f"Retry {retry}: waiting {wait_time:.0f}s before next attempt...")
            ...
            try:
                result = _download_image_post(
                    ...
                    model=try_model, seed=seed or (hash(prompt_text) % 10000) + retry * 1000,
