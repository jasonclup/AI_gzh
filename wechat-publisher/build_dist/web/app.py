...
def create_app(config: dict):
    ...
    def serve_pwa_icon(filename):
        ...
        allowed_exts = {'.png', '.jpg', '.svg', '.ico', '.webp'}
    ...
    def not_found(e):
        if request.accept_mimetypes.accept_html and not request.path.startswith('/api/'):
            try:
                tf2, ag2, ig2, wp2, sc2 = init_modules()
                today_stats = sc2.get_today_stats()
                recent_history = sc2.get_history(7)
                return render_template('dashboard.html',
                                     today_stats=today_stats,
                                     history=recent_history,
                                     wechat_configured=wp2.is_configured())
            except Exception:
                pass
